...
phenology label. Downstream scripts require four classes for wheat, rice, and
soybean, but three for maize. This legacy copy deliberately keeps its original
``n_clusters=4``; change that value to 3 before regenerating maize labels.
Labels are arbitrary identifiers, so retain the generated output table rather
than comparing their numeric values across independent clustering runs.
"""
...
labels = cluster.fit_predict(df.iloc[:, 1:3])
df['class'] = labels
