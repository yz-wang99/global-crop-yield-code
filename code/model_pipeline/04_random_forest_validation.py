...
all_results = []
all_grouped_results = []
def crossvalid(output_path, intervals, feature_combination, harvests, file_dates, crop, clf, feature, seed, model_save_dir):
    ...
    for i in class_num:
        ...
        # 删除 year 为 2002 或 2003 的行
        #data = data[~data['year'].isin([2002])]
...
output_path2022 = 'F:\\重要文档\\data\\test2022\\'
output_path2021 = 'F:\\重要文档\\data\\test2021\\'
output_path2020 = 'F:\\重要文档\\data\\test2020\\'

model_save_dir = r'E:\D盘WYZ\Process_country(Non_scale)\test2020\models'
    ...
    for seed in seed_list:
        ...
        for intervals, harvests, file_date in zip(intervals0, harvest0, file_dates):
            ...
            crossvalid(
                ...
                crop=crop_type,
                clf='RF',
                feature_tag=f"0{feature_combination}",
                seed=seed,
...
summary_df.to_csv(
    r'F:\重要文档\data\test2020\RF00_summary_mean.csv',
    index=False
)
...
grouped_summary_df.to_csv(
    r'F:\重要文档\data\test2020\RF00_grouped_summary_mean.csv',
    index=False
)
